...
#[test]
fn test_magic_enum() {
    let mut test = Cursor::new(b"\0\x02");
    assert_eq!(TestMagicEnum::read(&mut test).unwrap(), TestMagicEnum::Two);

    let mut test = Cursor::new(b"\0\x01");
    let err = TestMagicEnum::read(&mut test).expect_err("Invalid variant should not be read");
    assert!(matches!(err, binread::Error::NoVariantMatch { .. }));
}
...
#[test]
fn test_unit_enum() {
    let mut test = Cursor::new(b"\xff\xff");
    assert_eq!(TestUnitEnum::read(&mut test).unwrap(), TestUnitEnum::Neg1);

    let mut test = Cursor::new(b"\0\x02");
    assert_eq!(TestUnitEnum::read(&mut test).unwrap(), TestUnitEnum::Two);

    let mut test = Cursor::new(b"\0\x01");
    let err = TestUnitEnum::read(&mut test).expect_err("Invalid variant should not be read");
    assert!(matches!(err, binread::Error::NoVariantMatch { .. }));
}
