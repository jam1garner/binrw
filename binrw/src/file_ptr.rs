...
impl<Ptr: BinRead<Args = arg_type!(())> + IntoSeekFrom, BR: BinRead> BinRead for FilePtr<Ptr, BR> {
    type Args = BR::Args;
...
impl<Ptr: BinRead<Args = arg_type!(())> + IntoSeekFrom, T> FilePtr<Ptr, T> {
...
where
    Ptr: BinRead<Args = arg_type!(())> + IntoSeekFrom,
